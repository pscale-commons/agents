...
def has_active_purpose(purpose_block):
    """True if any digit-keyed entry in purpose is non-empty."""
    for k, v in purpose_block.items():
        if k == '_':
            continue
...
def find_ripe_concern(concerns_block, has_human_input, purpose_block=None, pending_compaction=False):
    ...
    for key in sorted(concerns_block.keys()):
        ...
        if trigger == 'always':
            ripe = has_active_purpose(purpose_block or {})
        elif trigger == 'human':
...
def main():
    ...
    while True:
        ...
        concern_key, concern_entry = find_ripe_concern(
            blocks.get('concerns', {}), has_human,
            blocks.get('purpose', {}), pending_compaction
        )
