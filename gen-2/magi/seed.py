#!/usr/bin/env python3
"""seed.py — pscale kernel v3. Tool_use API + two-channel compilation.
System prompt = constitution (unfolded). Message = concern + environment."""
...
def call_llm_with_tools(config, system, message):
    ...
    for iteration in range(10):  # A-loop safety limit
        _log(f"  API call {iteration}: model={model}")
...
def main():
    ...
    print(f"[KERNEL] Loaded {seed_path}")
