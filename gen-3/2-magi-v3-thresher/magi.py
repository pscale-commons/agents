...
def build_message(compiled):
    """Build message — the past phase content + future phase framing."""
    parts = []
    ...
    for key, info in sorted(compiled.items()):
        if info["block_name"] == "soul":
            continue  # soul is in system prompt
        formatted = format_compiled(info["compiled"])
        if info["block_name"] == "purpose":
            purpose_parts.append(formatted)
        elif info["block_name"] == "history":
    ...
    # Purpose
    if purpose_parts:
        parts.append("=== PURPOSE (the gap to close) ===")
        parts.extend(purpose_parts)
        parts.append("")

    # Content
    if content_parts:
        parts.append("=== CONTENT (material to operate on) ===")
        parts.extend(content_parts)
        parts.append("")

    # History
    if history_parts:
        parts.append("=== HISTORY (what has been attempted) ===")
    ...
    parts.append("")
    parts.append("If you need more detail from the content, modify the function")
    parts.append("block's star references: change mode from 'spindle' to 'dir'")
    parts.append("or 'ring' to see more. The next instance will perceive the result.")
